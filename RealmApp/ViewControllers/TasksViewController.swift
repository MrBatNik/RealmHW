    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let doneAction = UIContextualAction(style: .normal, title: title) { _, _, isDone in
            if indexPath.section == 0 {
                ...
                let row = self.completedTasks.isEmpty ? 0 : self.completedTasks.count - 1
                tableView.moveRow(at: IndexPath(row: indexPath.row, section: 0),
                                  to: IndexPath(row: row, section: 1))
            } else {
                StorageManager.shared.undone(task)
                let row = self.currentTasks.isEmpty ? 0 : self.currentTasks.count - 1
                tableView.moveRow(at: IndexPath(row: indexPath.row, section: 1),
                                  to: IndexPath(row: row, section: 0))
            }
